...
class EnergyPanel extends Phaser.GameObjects.Container {
	...
	constructor(scene, x, y) {
		...
		// selector
		const selector = scene.add.sprite(-26, -44, "energyPanel_selector");
		selector.scaleX = 0.6;
		selector.scaleY = 0.6;
		this.add(selector);

		// heartBtn (prefab fields)
		heartBtn.itemValue = 50;
	...
	//	this.cannonStrikeText.text =  this.biteBtn.itemValue;
		...
		this.selectorPositions=[-44,33,116]
		this.currentSelectorPos = 0;
		this.canSelect=false;

		var downKey = this.scene.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.DOWN);

		downKey.on('down', this.selectDown, this);

		var upKey = this.scene.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.UP);

		upKey.on('down', this.selectUp, this);

		var enterKey = this.scene.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ENTER);

		enterKey.on('down', this.choose, this);

		this.escPressed = this.scene.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ESC);
		this.escPressed.on('down', function(){
	...
	choose(){
		if(this.canSelect){
			switch(this.currentSelectorPos){
				case 0:
					this.comprarEnergyRode(this.energyRode.itemValue);
					break;
				case 1:
					this.comprarHeart(this.heartBtn.itemValue);
					break;
				case 2:
					this.comprarLifeRode(this.lifeRode.itemValue);
					break;
			}
		}
	...
	selectDown(){
		if(this.canSelect){
				...
				if(this.currentSelectorPos>=this.selectorPositions.length){
					this.currentSelectorPos=0;
				}
				this.selector.y=this.selectorPositions[this.currentSelectorPos];
		}
	...
	selectUp(){
		...
		if(this.currentSelectorPos<0){
			this.currentSelectorPos=this.selectorPositions.length-1;
		}
		this.selector.y=this.selectorPositions[this.currentSelectorPos];
	}
	}
...
comprarCannonRelay(itemValue){
	...
	close(){
		if(this.scene.player.wannaEnterPod){
			this.scene.tweens.add({
					targets: this,
					y:  this.scene.cameras.main.height+90,
					ease: 'Quadratic.Out',
