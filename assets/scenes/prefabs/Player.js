...
class Player extends Phaser.GameObjects.Sprite {
	...
	//	console.log(this.scene.game.playerData);
		...
		var shiftKey = this.scene.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SHIFT);

		spaceBar.on('down', this.spaceBarIsDown, this)
		spaceBar.on('up', this.spaceBarIsUP, this)


		shiftKey.on('down', this.shiftIsDown, this)
		shiftKey.on('up', this.shiftIsUP, this)
	...
	spaceBarIsDown(){
			...
			if(this.waitingfortarget){
				this.dispararseDeCannon(this.spriteKnob,this.spriteCircle.angle-90)
			}
if(!this.isFiredfromCannon && !this.waitingfortarget){
	...
	enableKeyBoard(){
			...
			if (this.cursors.left.isDown && !this.waitingfortarget)
		{
		...
		else if (this.cursors.right.isDown  && !this.waitingfortarget)
		{	if(!this.isJetPackActive){
				this.KeyBoardPower+=this.KeyboardIncreasPowerX;
			if(this.KeyBoardPower>this.maxKeyWalkPower){
				this.KeyBoardPower=this.maxKeyWalkPower;
	...
	dispararseDeCannon(gameObject,deg){
	...
	updatePlayer() {
		...
		if (this.isJetPackActive && !this.isusingKeyBoard) {
