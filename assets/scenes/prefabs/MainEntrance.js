...
class MainEntrance extends Phaser.GameObjects.Container {
	...
	prepareToEnter(player, entrance){
		...
		if(this.canUseEnter){
			this.downKey.on('down', function(){
					...
					player.scene.cameras.main.fadeOut(1000);
					player.scene.cameras.main.once('camerafadeoutcomplete', function (camera) {
						
					var sceneToGo = player.scene.scene.get("LevelMap");
					sceneToGo.setLevel("NewLevel1A",1,0); //nombre de la escena a cargar, casillero en el mapa para trasladarse y casillero donde debe partir
					sceneToGo.isMainScene = true;
					player.scene.scene.remove(player.scene.keys);
					player.scene.scene.start("LevelMap");
